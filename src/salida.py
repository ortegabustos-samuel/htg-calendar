...
# --------------------------------------------------------------------------- #
#  Vista del PIPELINE v3 (src/v3/pipeline.py) — no toca nada de lo de arriba
# --------------------------------------------------------------------------- #
def _bloques_v3(datos: Datos) -> list[tuple[str, list[str]]]:
    """Orden simple de filas para la vista de v3: fijos, patrones (agrupados por patrón), mixtos,
    correturnos. Sin la lógica de "dedicados"/pueblos de `_bloques` (esa usa `turno.prioridad`,
    que no existe en el modelo de datos de v3)."""
...
def _cobertura_v3(plan: dict[tuple[str, date], str]) -> dict[tuple[str, date], int]:
...
def escribir_excel_v3(datos: Datos, plan: dict[tuple[str, date], str],
                      nombre: str = "calendario_v3.xlsx") -> Path:
    """Vuelca el cuadrante de v3 a XLSX, con la misma estructura que la hoja del motor anterior.
    ...
    titulos: set[int] = set()
    r = HDR + 1
    for titulo, gente in _bloques_v3(datos):
        c = ws.cell(r, 1, titulo)
        c.font = Font(bold=True, size=12)
        titulos.add(r)
        r += 1
